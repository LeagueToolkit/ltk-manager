...
fn a_modpkg_readme_is_read_off_disk_without_mounting_anything() {
    ...
    let document = read_readme(storage.path(), &entry).unwrap();
...
fn a_fantome_readme_is_read_off_disk_once_it_has_been_extracted() {
    ...
    read_readme(storage.path(), &entry).unwrap();
...
fn a_fantome_license_is_read_out_of_the_archive_and_not_written_beside_the_mod() {
    ...
    let document = read_license(storage.path(), &entry).unwrap();
...
fn a_modpkg_license_is_read_out_of_its_own_chunk() {
    ...
    assert_eq!(
        read_license(storage.path(), &entry).unwrap(),
        ModDocument::Present {
...
#[test]
fn a_mod_whose_archive_was_not_kept_is_not_reported_as_undocumented() {
    let storage = tempfile::tempdir().unwrap();
